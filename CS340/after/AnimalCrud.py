...
# Declare global variables to store cached data
cached_data = {
    "R": None,
    "WR": None,
    "MR": None,
...
def update_cache(value, df):
    cached_data[value] = df
...
def update_datatable(value):
    ...
    if cached_data[value] is not None:
        print(f"Using cached data for {value}")
        return cached_data[value].to_dict("records")

    if value == "R":
        df = pd.DataFrame.from_records(aac.read_all())
    elif value == "WR":
    ...
    update_cache(value, df)
    print(f"Filtered to {value} \n {df.head(5)}")
